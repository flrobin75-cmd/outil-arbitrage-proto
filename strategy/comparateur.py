...
MODE_AUDIT (G3d, spec 1.1.0) :
- `calcul_comparateur()` accepte un paramètre opt-in `audit: TraceAudit | None`.
  Codes émis : `COMP_*` (namespace dédié, isolé des `STRAT_*` parce que ce
  module est un comparateur/réceptacle, pas une stratégie mono-régime).
- **Aucune sous-trace attachée** : le module est autonome (aucun import depuis
  `regime/*` ni `strategy/{tns,liberal,assimile}.py`). La trace G3d est plate,
  structurée uniquement par `parent_id`.
- Trace découpée :
  * G3d.1 : 6 étapes section A (paramètres dérivés)
  * G3d.2 : 15 étapes Lignes (`COMP_LIGNE_<NN>_*`) + 4 étapes Top 3 (`COMP_TOP3_*`)
  * G3d.3 : 4 étapes Réceptacles + 4 étapes Alertes
- Vocabulaire factuel strict : `COMP_TOP3_RANG_*` (rang dans classement par
  score, pas « gagnant »), `COMP_ALERTES_NB` (count), pas de wording prescriptif.
- Textes d'alertes métier (lectures URSSAF, plafonds dépassés) préservés en
  `hypotheses` selon le pattern G3b/G3c.
"""
...
def calcul_comparateur(profil: Profil, config: ConfigComparateur,
                       remuneration_brute_pour_pero: Optional[float] = None,
                       *,
                       audit: TraceAudit | None = None) -> ResultatComparateur:
    """
    ...
    Args:
        ...
        audit: Trace d'audit optionnelle (MODE_AUDIT G3d). Side channel —
            n'affecte jamais le résultat numérique. Codes émis : `COMP_*`.
            Le comparateur étant autonome (aucun import régime/stratégie
            instrumenté), la trace est plate et structurée par parent_id.
    """
    ...
    _log("COMP_REVENU_IMPOSABLE_PAR_PART",
         "Revenu imposable par part (heuristique v19 enveloppe × ratio)",
         revenu_par_part, unite="EUR",
         hypotheses={"regime_social_profil": profil.regime_social,
                     "ratio_net_imposable_applique": RATIO_NET_IMPOSABLE.get(profil.regime_social, 0.55),
                     "enveloppe_profil": profil.enveloppe,
                     "autres_revenus_profil": profil.autres_revenus,
                     "parts_profil": profil.parts},
         notes="Heuristique v19 — base de calcul pour la TMI estimée")
    _log("COMP_TMI_ESTIMEE",
         "TMI estimée selon barème pour le revenu par part",
         tmi, unite="ratio",
         doctrine_refs=("IR_PLAFOND_T1", "IR_PLAFOND_T2",
                        "IR_PLAFOND_T3", "IR_PLAFOND_T4",
                        "IR_TAUX_T2", "IR_TAUX_T3",
         ...
         hypotheses={"IR_PLAFOND_T1": IR_PLAFOND_T1,
                     ...
                     "IR_TAUX_T2": IR_TAUX_T2,
                     "IR_TAUX_T3": IR_TAUX_T3,
                     "IR_TAUX_T4": IR_TAUX_T4,
                     "IR_TAUX_T5": IR_TAUX_T5,
                     "revenu_par_part_calcule": revenu_par_part})
    _log("COMP_FS_PARTICIPATION",
         "Forfait social applicable à la participation (selon effectif)",
         fs_part, unite="ratio",
         hypotheses={"effectif_profil": profil.effectif,
                     "table_fs_participation": dict(FS_PARTICIPATION)})
    _log("COMP_FS_INTERESSEMENT",
         "Forfait social applicable à l'intéressement (selon effectif)",
         fs_int, unite="ratio",
         hypotheses={"effectif_profil": profil.effectif,
                     "table_fs_interessement": dict(FS_INTERESSEMENT)})
    _log("COMP_FS_ABO_PEE",
         "Forfait social applicable à l'abondement PEE (selon effectif)",
         fs_abo_pee, unite="ratio",
         hypotheses={"effectif_profil": profil.effectif,
                     "table_fs_abo_pee": dict(FS_ABO_PEE)})
    ...
    _log("COMP_MONTANT_PERO",
         "Montant PERO calculé (mode pourcentage ou euros)",
         montant_pero, unite="EUR",
         doctrine_refs=("TX_PATRONAL",),
         hypotheses={"TX_PATRONAL": TX_PATRONAL,
                     "pero_actif_config": config.pero_actif,
                     "dirigeant_eligible_pero": config.dirigeant_eligible_pero,
                     "pero_mode_saisie": config.pero_mode_saisie,
                     "pero_taux_config": config.pero_taux,
                     "pero_montant_config": config.pero_montant,
                     "remuneration_brute_pour_pero": remuneration_brute_pour_pero},
         notes="Montant nul si PERO inactif ou dirigeant non éligible")
    ...
    # === MODE_AUDIT G3d.2 — Trace des lignes et du top 3 ===
    # Mapping factuel idx → nom court pour le label de l'étape (label épuré,
    # nom intégral préservé en hypotheses)
    _NOMS_COURTS_LIGNES = {
        0: "salaire", 1: "dividendes",
        2: "participation", 3: "interessement",
        4: "abondement_pee", 5: "abondement_pereco",
        6: "perin", 7: "avantages_nature",
        8: "tickets_restaurant", 9: "cesu",
        10: "cheques_cadeaux", 11: "mutuelle",
        12: "indemnites_kilometriques", 13: "cashback",
        14: "pero",
    }

    if audit is not None:
        # Agrégat de tête : nombre de lignes (factuel)
        audit.add("COMP_NB_LIGNES",
                  "Nombre de lignes dispositifs construites",
                  float(len(lignes)), unite="count",
                  hypotheses={"plage_idx": "0 à 14",
                              "lignes_inconditionnelles": "salaire (0), dividendes (1)",
                              "lignes_conditionnelles": "2 à 14 (activables par config)"})

        # Trace par ligne (idx 0 à 14)
        for idx, ligne in enumerate(lignes):
            nom_court = _NOMS_COURTS_LIGNES.get(idx, f"ligne_{idx}")
            code = f"COMP_LIGNE_{idx:02d}_{nom_court.upper()}"
            audit.add(
                code,
                f"Ligne {idx:02d} — {nom_court} (net après IR)",
                ligne.net_apres_ir, unite="EUR",
                parent_id="COMP_NB_LIGNES",
                hypotheses={
                    "nom_dispositif_integral": ligne.nom,
                    "active": ligne.active,
                    "montant_input": ligne.montant_input,
                    "cout_societe": ligne.cout_societe,
                    "cout_beneficiaire": ligne.cout_beneficiaire,
                    "cotis_ps": ligne.cotis_ps,
                    "net_imposable": ligne.net_imposable,
                    "ir_estime": ligne.ir_estime,
                    "ratio_net_cout": ligne.ratio_net_cout,
                    "coef_risque": ligne.coef_risque,
                    "score_ajuste": ligne.score_ajuste,
                    "accessible": ligne.accessible,
                    "motif_inaccessibilite": ligne.motif_inaccessibilite,
                    "top3_rang": ligne.top3_rang,
                })

        # Top 3 — discipline non-prescriptive : labels strictement mécaniques
        audit.add("COMP_TOP3_CRITERE",
                  "Critère de classement des dispositifs",
                  "tri décroissant sur score_ajuste = ratio_net_cout × coef_risque",
                  unite="",
                  hypotheses={"score_formule": "K × L",
                              "regle_egalite": "priorité à la ligne au-dessus dans le tableau"},
                  notes="Classement mécanique sur scores. Ne reflète pas un avis "
                        "sur le choix de dispositif — interprétation par l'EC.")

        lignes_avec_rang = [(idx, lig) for idx, lig in enumerate(lignes)
                            if lig.top3_rang is not None]
        audit.add("COMP_TOP3_NB",
                  "Nombre de lignes classées dans le top 3 (0 à 3)",
                  float(len(lignes_avec_rang)), unite="count",
                  hypotheses={"lignes_eligibles_idx": list(lignes_eligibles),
                              "lignes_eligibles_actives": sum(
                                  1 for i in lignes_eligibles
                                  if lignes[i].active == "Oui"),
                              "convention_classement": "Salaire et dividendes exclus, "
                                                       "lignes inaccessibles exclues"})

        # Trace par ligne classée (rang 1, 2, 3 selon les cas)
        # Tri pour ordre stable rang 1 → 2 → 3
        for idx, ligne in sorted(lignes_avec_rang,
                                 key=lambda t: t[1].top3_rang):
            nom_court = _NOMS_COURTS_LIGNES.get(idx, f"ligne_{idx}")
            code = f"COMP_TOP3_RANG_{ligne.top3_rang}"
            audit.add(
                code,
                f"Rang {ligne.top3_rang} dans le classement par score",
                float(ligne.top3_rang), unite="rang",
                parent_id="COMP_TOP3_CRITERE",
                hypotheses={
                    "idx_ligne": idx,
                    "nom_dispositif_integral": ligne.nom,
                    "nom_court": nom_court,
                    ...
                    "ratio_net_cout": ligne.ratio_net_cout,
                    "coef_risque": ligne.coef_risque,
                    "net_apres_ir": ligne.net_apres_ir,
                    "cout_societe": ligne.cout_societe,
                })
    ...
    # === MODE_AUDIT G3d.3 — Trace des réceptacles et alertes ===
    if audit is not None:
        # Agrégat réceptacles
        audit.add("COMP_NB_RECEPTACLES",
                  "Nombre de vues réceptacles construites",
                  float(len(receptacles)), unite="count",
                  hypotheses={"receptacles_attendus": ["PEE", "PERECO", "PERO", "PERIN"]})

        # Trace par réceptacle
        for rec in receptacles:
            code = f"COMP_RECEPTACLE_{rec.nom}"
            audit.add(
                code,
                f"Réceptacle {rec.nom} — montant total des flux entrants",
                rec.montant_total, unite="EUR",
                parent_id="COMP_NB_RECEPTACLES",
                hypotheses={
                    "actif": rec.actif,
                    "flux_entrants": rec.flux_entrants,
                    "plafond_legal": rec.plafond_legal,
                    "plafond_label": rec.plafond_label,
                    "taux_utilisation": rec.taux_utilisation,
                    "statut_pictogramme": rec.statut,
                    "message_alerte_si_depassement": rec.message,
                })

        # Alertes : counts factuels par sévérité, textes intégraux en hypotheses
        nb_error = sum(1 for a in alertes if a.severite == "error")
        nb_warning = sum(1 for a in alertes if a.severite == "warning")
        nb_info = sum(1 for a in alertes if a.severite == "info")

        # Sérialisation des alertes pour préservation du wording métier
        alertes_serialisees = [
            {"severite": a.severite, "titre": a.titre, "message": a.message}
            for a in alertes
        ]

        audit.add("COMP_ALERTES_NB",
                  "Nombre total d'alertes construites",
                  float(len(alertes)), unite="count",
                  hypotheses={
                      "ventilation_par_severite": {
                          "error": nb_error,
                          "warning": nb_warning,
                          "info": nb_info,
                      },
                      "textes_alertes_integraux": alertes_serialisees,
                  },
                  notes="Wording métier des alertes préservé intégralement en hypotheses "
                        "(pattern G3b/G3c — pas dans le label/notes)")
        audit.add("COMP_ALERTES_ERROR_NB",
                  "Nombre d'alertes de sévérité 'error'",
                  float(nb_error), unite="count",
                  parent_id="COMP_ALERTES_NB")
        audit.add("COMP_ALERTES_WARNING_NB",
                  "Nombre d'alertes de sévérité 'warning'",
                  float(nb_warning), unite="count",
                  parent_id="COMP_ALERTES_NB")
        audit.add("COMP_ALERTES_INFO_NB",
                  "Nombre d'alertes de sévérité 'info'",
                  float(nb_info), unite="count",
                  parent_id="COMP_ALERTES_NB")
